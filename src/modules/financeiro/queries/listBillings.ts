...
import type { Billing, BillingDerivedStatus, BillingType } from '@/types/domain';
...
export async function listBillings({
  ...
  let query = supabase
    .from('billings')
    .select(
      'id, org_id, patient_id, appointment_id, type, description, amount_cents, status, due_date, paid_at, payment_method, created_at, updated_at, patients(full_name)',
      { count: 'exact' },
    )
